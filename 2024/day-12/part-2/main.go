...
func getPerimeter(i int, j int, lines []string) int {
	p := 0
	if (!isAvailable(i, j+1, lines) || lines[i][j] != lines[i][j+1]) && (!isAvailable(i-1, j, lines) || lines[i][j] != lines[i-1][j]) {
		p++
	}
	if (!isAvailable(i, j+1, lines) || lines[i][j] != lines[i][j+1]) && (!isAvailable(i+1, j, lines) || lines[i][j] != lines[i+1][j]) {
		p++
	...
	if isAvailable(i+1, j, lines) && isAvailable(i, j-1, lines) && lines[i][j] == lines[i+1][j] && lines[i][j] == lines[i][j-1] && lines[i][j] != lines[i+1][j-1] {
		p++
	}
	if isAvailable(i+1, j, lines) && isAvailable(i, j+1, lines) && lines[i][j] == lines[i+1][j] && lines[i][j] == lines[i][j+1] && lines[i][j] != lines[i+1][j+1] {
		p++
