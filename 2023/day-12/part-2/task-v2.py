...
def find(schema, requirements, current_position, current_block_position, block_size):
	...
	if schema[current_position] == '.' or schema[current_position] == '?':
		if block_size == 0:
			sum += find(schema, requirements, current_position + 1, current_block_position, 0)
		elif (block_size > 0 and
		      current_block_position < len(requirements) and
		      requirements[current_block_position] == block_size):
			sum += find(schema, requirements, current_position + 1, current_block_position + 1, 0)

	if schema[current_position] == '#' or schema[current_position] == '?':
		sum += find(schema, requirements, current_position + 1, current_block_position, block_size + 1)
